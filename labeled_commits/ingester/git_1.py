...
class Git():
    ...
    def getDiffStatsProperties(self, stats, commitFiles, devExperience, author, unixTimeStamp):
        ...
        for stat in stats:
            ...
            if file_m['File'] and "__init__.py" not in file_m['File']:
                ...
                if type_f:
                    if type_f[-1] in set(["c", "cc", "cxx", "C", "cpp", "CPP",
        ...
        return statProperties, files_metrics
    ...
    def getCommitStatsProperties(self, stats, commitFiles, devExperience, author, unixTimeStamp):
        ...
        for stat in stats:
            file_m = {"File": "", "la": 0, "ld": 0, "ns": 0, "nd": 0, "nuc": 0,
                      "entrophy": 0, "lt": 0, "ndev": 0, "age": 0,
                      "exp": 0, "rexp": 0, "sexp": 0, "fix": 0}
        ...
        for fileLocMod in locModifiedPerFile:
            if fileLocMod != 0:
                ...
                entrophy -= (avg * math.log(avg, 2))
        #print("testing entrophy", temp_entrophy, entrophy)
        # Add stat properties to the commit objectjson_file
        statProperties += ',"la":"' + str(la) + '\"'
        statProperties += ',"ld":"' + str(ld) + '\"'
        statProperties += ',"fileschanged":"' + filesSeen[0:-1] + '\"'
        statProperties += ',"nf":"' + str(nf) + '\"'
    ...
    def log(self, repo, firstSync, json_file):
        """
        ...
        description: a very basic abstraction for using git in python.
        """
        dir_temp = os.path.dirname(__file__) + self.REPO_DIRECTORY + repo.id
        repo_dir = os.chdir(dir_temp)
        #print(os.getcwd())
        ...
        for commit in commitList:
            ...
            temp_commit_obj, file_objs = self.getCommitStatsProperties(stats, commitFiles, devExperience, author, unixTimeStamp)
            #pdb.set_trace()
            # Update the classification of the commit
            commitObject += temp_commit_obj
            commitObject += ',"classification":"' + str(classification) + '\"'

            # Update whether commit was a fix or not
            if file_objs and fix:
            ...
            json_list[-1]['files'] = file_objs
            #pdb.set_trace()
        # End commit loop

        logging.info('Done getting/parsing git commits.')
        #with open(repo.id + '_commits.json', 'w') as outfile:
...
if __name__ == "__main__":
    ...
    #print(repo.id)
    adapter = Git()
    #p_commits = json.load(open(adapter.REPO_DIRECTORY + repo_id + "/%s"%repo_id + "_commits.json"))
    commits = adapter.log(repo, False, None)
    #pdb.set_trace()
